...
## 4. ドキュメント管理
- **真実のソース**: `docs/tasks/` 以下のファイルは常に最新の状態を保ってください。
- **役割分担**:
    - `progress.md`: 現在進行中のフェーズと直近完了したフェーズのサマリー。
    - `completed.md`: 完了した全フェーズの恒久的な詳細記録。
    - `phases/*.md`: 各フェーズの具体的な要件とタスク詳細。
- **整理ルール**:
    - フェーズが完了した際、または新しいフェーズが開始される際、`progress.md` にある完了済みフェーズの詳細を `completed.md` へ移管し、`progress.md` を軽量に保ってください。
