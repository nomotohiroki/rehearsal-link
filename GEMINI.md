...
2.  **原子的な作業（重要）**: **一度に複数のタスク（あるいはフェーズ全体）をまとめて実装することは厳禁です。** 必ず「1つのタスク（最小単位）に対して1つの修正・確認サイクル」を徹底してください。
...
7.  **レビュー**:
    ...
    - リリースが必要な場合は、GitHubのリリースページから手順に従ってリリースを行ってください（詳細は `docs/tasks/phases` 内の指示や `scripts/` を参照）。
